  ...
  <header class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center shrink-0 px-1">
   <div>
    <h1 class="text-3xl font-bold tracking-tight text-foreground lg:text-4xl">
     Portal <span class="bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent"
      >AION</span
     >
    </h1>
    <p class="mt-1.5 flex items-center gap-2 text-sm text-muted-foreground">
     ...
     Sesi aktif:
     <span class="font-medium text-foreground">{$page.props.user?.username || 'Administrator'}</span
     >
    </p>
   </div>
   ...
   >
    {#if form.processing}
     <span
      class="h-4 w-4 animate-spin rounded-full border-2 border-destructive border-t-transparent"
     ></span>
       ...
       <div in:fly={{ y: 20, duration: 400, delay: i * 100 }}>
        ...
        >
         <div class="flex items-center gap-4">
          <div
           class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br {card.color} shadow-inner transition-transform group-hover:scale-110"
          >
         ...
         <div
          class="mt-auto flex items-center gap-2 text-xs font-medium text-primary/80 group-hover:text-primary transition-colors"
         >
     ...
     >
      ...
      <dl class="space-y-4">
       <div class="flex items-center justify-between border-b border-border/50 pb-3">
        ...
        <dd class="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
         {$page.props.user?.role || 'Super Admin'}
        </dd>
       </div>
