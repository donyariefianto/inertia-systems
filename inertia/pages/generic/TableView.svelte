      ...
      >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
          {#each config.fields || [] as field}
            {#if field.show_up !== false && field.name !== 'id'}
              <div class="space-y-2 group animate-in fade-in slide-in-from-right-4 duration-300">
                <label
                  for="Currency"
                  class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70 flex items-center justify-between group-focus-within:text-primary transition-colors"
                >
                  <span>
                    {field.label}
                  ...
                  {#if field.type === 'currency'}
                    <span class="text-[8px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground"
                      >IDR</span
                    >
                  {/if}
                </label>
                ...
                {:else if field.type === 'repeater'}
                  <div
                    class="p-4 border-2 border-dashed border-border rounded-2xl bg-muted/5 space-y-4"
                  >
                    <div class="flex items-center justify-between">
                      <span class="text-[10px] font-black text-primary uppercase tracking-widest"
                        >{field.label} List</span
                      >
                      <button
                        type="button"
                        onclick={() => {
                        ...
                        class="text-[9px] font-bold bg-primary/10 text-primary px-2.5 py-1 rounded-lg hover:bg-primary hover:text-white transition-all"
                      >
                    ...
                    {#if !formData[field.name] || formData[field.name].length === 0}
                      <p class="text-[10px] text-center text-muted-foreground py-2 italic">
