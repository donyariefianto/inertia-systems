<script lang="ts">
 ...
 const techStack = $derived([
  ...
  {
   tech: t('landing.tech.categories.infrastructure'),
   name: 'Docker & Compose',
   icon: 'box',
  },
   ...
   {#each pillars as category}
    <div class="flex flex-col">
     <div class="flex items-center gap-3 mb-6">
      ...
      <h3 class="text-lg font-bold text-foreground whitespace-nowrap">
       {category.category}
      </h3>
      <div class="h-px flex-1 bg-border/80"></div>
     </div>
     ...
     <div class="space-y-4">
      {#each category.items as item}
       ...
       >
        ...
        <p class="text-sm text-muted-foreground mt-2 ml-6 leading-relaxed">
         {item.desc}
        </p>
       </div>
      {/each}
  ...
  >
   ...
   <div class="flex gap-4">
    ...
    <a
     ...
     class="text-muted-foreground hover:text-foreground"><i class="fab fa-linkedin text-xl"></i></a
    >
   </div>
  </div>
 </div>
