...
{#if visible}
  ...
  >
    {#if flash?.success}
      <div class="pointer-events-auto flex items-center gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-4 text-emerald-500 shadow-lg backdrop-blur-md">
        <i class="fas fa-check-circle"></i>
        <p class="text-sm font-medium">{flash.success}</p>
        <!-- svelte-ignore a11y_consider_explicit_label -->
        <button type="button" onclick={() => (visible = false)} class="ml-auto opacity-50 hover:opacity-100">
    ...
    {#if flash?.error}
      <div class="pointer-events-auto flex items-center gap-3 rounded-lg border border-destructive/20 bg-destructive/10 p-4 text-destructive shadow-lg backdrop-blur-md">
